...
| JetBrains Mono as blanket "dev" font | Mono is for *technical* content — ports, commands, URLs. Names go in Montserrat sans. |
...
**The design system is skinnable.** All colors, typography, and tokens live in a single source of truth — [`references/style-guide.md`](references/style-guide.md). This file describes semantic roles (`paper`, `ink`, `muted`, `accent`, `link`, …). This project ships with the Bifunctional skin; edit `style-guide.md` directly or run the URL-based flow described in [`references/onboarding.md`](references/onboarding.md) when creating a separate skin.
...
- **Title** — Fraunces, 1.75rem, 400 — H1 only
- **Node name** — Montserrat (sans), 12px, 600 — human-readable labels
- **Sublabel** — system mono, 9px — ports, URLs, field types
- **Eyebrow / tag** — Montserrat, 7–8px, uppercase, tracked — type tags, axis labels
- **Arrow label** — system mono, 8px — annotation on arrows
- **Editorial aside** — Fraunces *italic*, 14px — callouts only

**Mono is for technical content.** Names are Montserrat sans. Page title and annotation callouts use Fraunces. Never use mono as a blanket "dev" font.
...
```svg
<rect width="100%" height="100%" fill="#efede7"/>
```
...
<marker id="arrow" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#00634d"/>
</marker>
<marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3" orient="auto">
  <polygon points="0 0, 8 3, 0 6" fill="#ff4f81"/>
</marker>
...
| Arrow | Stroke | When |
|---|---|---|
| Default | muted `#00634d` | Internal, generic |
| Accent | accent `#ff4f81` | Primary / highlighted / headline |
...
<rect x="X" y="Y" width="W" height="H" rx="6" fill="#efede7"/>
<!-- 2. Styled box -->
...
<text x="X+22" y="Y+15" fill="STROKE@0.8" font-size="7" font-family="ui-monospace, monospace"
      text-anchor="middle" letter-spacing="0.08em">API</text>
<!-- 4. Node name (Montserrat sans — human-readable) -->
<text x="CX" y="CY+2" fill="#032b22" font-size="12" font-weight="600"
      font-family="'Montserrat', sans-serif" text-anchor="middle">Node Name</text>
<!-- 5. Technical sublabel (system mono) -->
<text x="CX" y="CY+18" fill="#00634d" font-size="9"
...
<rect x="MID_X-18" y="ARROW_Y-20" width="36" height="12" rx="2" fill="#efede7"/>
<text x="MID_X" y="ARROW_Y-11" fill="#7a8399" font-size="8"
      font-family="ui-monospace, monospace" text-anchor="middle" letter-spacing="0.06em">WRITE</text>
```
...
<text x="30" y="LEGEND_Y+8" fill="#00634d" font-size="8" font-family="ui-monospace, monospace"
...
1. **Header** — eyebrow (system mono), title (Fraunces), optional subtitle (Montserrat muted).
...
4. **Footer** — colophon in system mono, muted, hairline top border.
...
- [ ] Every arrow label has an opaque `fill="#efede7"` rect behind it?
...
- [ ] Human-readable names in Montserrat sans, not system mono?
- [ ] Technical sublabels (ports, commands, URLs) in system mono?
- [ ] Page title in Fraunces?
- [ ] Annotation callouts (if any) in *italic* Fraunces? (see [primitive-annotation.md](references/primitive-annotation.md))
