...
nodes:                              # explicit per-cell entries; empty cells render nothing
  ...
  - { lane: "ADM", step: 1, title: "Access Control",  sub: "bucket policies · LDAP",    tool: "MinIO · LDAP console",
      color: "#c15b80" }            # tinted muted pink to flag governance/identity concern
...
- `#c15b80` muted pink — Security / Identity / Governance (admin nodes, LDAP, access control)
...
- `assets/example-data-flow-extended.html` — exercises §4 color override: Access Control node in muted pink (governance), Clean & Stage node in slate-blue (data quality). Focal accent on Analyze step + Explore & Model node + anon-data arrow unchanged.
