...
- **Left label column:** x=20–200 (180px). Task names in Montserrat sans 11px 600. Phase labels as system mono 7px eyebrows above each group.
...
- **Phase grouping:** a subtle zone rect (same pattern as architecture zone) behind each phase's rows, with an eyebrow label in the top-left margin. Use `rgba(3,43,34,0.02)` fill, `rgba(3,43,34,0.10)` stroke.
...
<rect x="X_start" y="ROW_Y+8" width="DURATION_PX" height="24" rx="4"
      fill="rgba(0,99,77,0.15)" stroke="#00634d" stroke-width="1"/>
<text x="X_start+8" y="ROW_Y+25" fill="#032b22" font-size="10" font-weight="600"
      font-family="'Montserrat', sans-serif">Task name</text>

<!-- Focal task -->
<rect x="X_start" y="ROW_Y+8" width="DURATION_PX" height="24" rx="4"
      fill="rgba(255,79,129,0.12)" stroke="#ff4f81" stroke-width="1"/>
<text x="X_start+8" y="ROW_Y+25" fill="#ff4f81" font-size="10" font-weight="600"
      font-family="'Montserrat', sans-serif">Key task</text>
```
