...
footer:                             # 0..N cross-cutting bars stacked below zone (full-canvas width)
  - { name: "Active Directory", icon: key,        subtitle: "LDAP · SSO · group RBAC",
      color: "#c15b80" }            # tinted red to flag the security concern
...
focal_accent: "#ff4f81"             # one color for all focal components (default = SKILL accent)
dark: false
```
...
Bars span the full zone width minus 16-px padding on each side. Bars marked `focal: true` use `bar_h_focal=56` and accent styling (fill `rgba(focal_accent, 0.08)`, stroke `focal_accent`). Non-focal bars use `bar_h_default=44` with muted styling (fill `rgba(3,43,34,0.05)`, stroke `rgba(3,43,34,0.32)`).
...
All side-column nodes use fixed `w=160 h=64`. Same fill / stroke pattern: fill `rgba(0,99,77,0.06)`, stroke `#4b6f65`, stroke-width 1.
...
|---|---|---|---|---|---|
| `primary` | `#ff4f81` (focal_accent) | 1.4 | — | `arrow-accent` | Every edge whose endpoint is a `focal: true` component. Also every Trino → consumer edge (serve-flow rule). |
| `secondary` | `#00634d` (muted) | 1.2 | — | `arrow` | Default for internal platform-component edges and source → platform edges that don't touch focal. |
| `federated` | `#a33c63` (link-blue) | 1.0 | `4,3` | `arrow-link` | Federation queries (e.g., source DB → Trino). |
| `trigger` | `#00634d` (muted) | 1.0 | `4,3` | `arrow` | Every edge originating from a `kind: bar` component (Airflow drops). Unlabelled. |
| `auth` | `#ff4f81` | 1.2 | `5,4` | `arrow-accent` | Every edge from a footer node up to the zone bottom edge. **Never to a specific component.** |
...
<defs>
  ...
  <marker id="arrow-link"   markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#a33c63"/></marker>
...
- `#c15b80` muted pink — Security / Identity (AD, Keycloak, Vault)
...
| Token | Light | Dark |
|---|---|---|
| Page paper | `#efede7` | `#032b22` |
| Ink | `#032b22` | `#efede7` |
| Muted | `#00634d` | `#9eb3aa` |
| Accent | `#ff4f81` | `#ff83a7` |
| Link (federated) | `#a33c63` | `#6a95d8` |
| Side-column fill | `rgba(0,99,77,0.06)` | `rgba(239,237,231,0.06)` |
| Side-column stroke | `#4b6f65` | `rgba(239,237,231,0.30)` |
| Zone fill | `rgba(3,43,34,0.025)` | `rgba(239,237,231,0.04)` |
| Zone stroke | `rgba(3,43,34,0.32)` | `rgba(239,237,231,0.30)` |
| Non-focal bar fill | `rgba(3,43,34,0.05)` | `rgba(239,237,231,0.06)` |
...
- `assets/example-dp-integration-extended.html` — exercises §4 color override + multi-footer: AD in muted pink, Observability (Prometheus/Grafana/Loki) in slate-blue. Canvas height grown to fit 2 footer rows.
