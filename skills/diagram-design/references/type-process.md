...
nodes:                              # explicit per-cell entries; empty cells render nothing
  ...
  - { lane: "FLD", step: 3,  title: "Train enumerators",  sub: "protocols · safety",       tool: "manual",
      chips: {in: "TB", out: "LS"}, color: "#c15b80" }    # muted pink — governance / training
  # ... etc
...
- Dot pattern: 22×22 grid, `circle r=0.8`, `fill rgba(3,43,34,0.10)`. Opacity 0.55.
- Alternating lane tints: odd-indexed lanes (0, 2, …) receive `rgba(3,43,34,0.018)` fill from `x=140` to `viewBox_w`.
- Lane dividers: horizontal hairlines at every `lane_y_top(k)` and at `legend_y_top`; stroke `rgba(3,43,34,0.12)` width 0.8.
- Label column right border: vertical hairline at `x = label_col_w`, stroke `rgba(3,43,34,0.20)` width 1, from `y = header_h` to `y = legend_y_top`.
...
- Default fill: `rgba(3,43,34,0.12)`, number text ink.
- Focal fill: `rgba(255,79,129,0.20)`, number text accent (§5).
...
- Default fill: muted (`#00634d` light / `#9eb3aa` dark).
- Focal fill: accent (`#ff4f81` light / `#ff83a7` dark).
...
|---|---|---|---|---|---|
| `normal` | `#00634d` (muted) | 1.0 | — | `arrow` | Standard data hand-off between steps or actors. Unlabelled. |
| `focal-in` / `focal-out` | `#ff4f81` (accent) | 1.2 | — | `arrow-accent` | Every edge whose endpoint is the focal node (`focal-in`) or origin is the focal node (`focal-out`). |
| `trigger` | `#00634d` (muted) | 1.0 | `4,3` | `arrow-sm` | Orchestration trigger (scheduler → tool, manual override → upstream step). Unlabelled. |
...
<defs>
  <pattern id="dots" width="22" height="22" patternUnits="userSpaceOnUse">
    <circle cx="11" cy="11" r="0.8" fill="rgba(3,43,34,0.10)"/>
  </pattern>
...
`C_light` = the same hex lightened ~15% for dark-mode contrast (e.g., `#c15b80` → `#d97a78`).
...
- `#c15b80` muted pink — Security / Identity / Governance (access control, training, approvals)
...
| Muted | `#00634d` | `#9eb3aa` |
| Soft | `#4b6f65` | `#a9c4b9` |
| Accent | `#ff4f81` | `#ff83a7` |
| Dot pattern | `rgba(3,43,34,0.10)` | `rgba(239,237,231,0.10)` |
| Lane tint | `rgba(3,43,34,0.018)` | `rgba(239,237,231,0.025)` |
| Dividers | `rgba(3,43,34,0.12)` | `rgba(239,237,231,0.12)` |
| Label col divider | `rgba(3,43,34,0.20)` | `rgba(239,237,231,0.22)` |
| Default chip fill | `rgba(3,43,34,0.12)` | `rgba(239,237,231,0.12)` |
| Focal chip fill | `rgba(255,79,129,0.20)` | `rgba(255,131,167,0.22)` |
| Default node fill | white | `rgba(239,237,231,0.04)` |
| Default node stroke | `rgba(3,43,34,0.25)` | `rgba(239,237,231,0.20)` |
| Focal node fill | `rgba(255,79,129,0.08)` | `rgba(255,131,167,0.12)` |
| Focal node stroke | `#ff4f81` | `#ff83a7` |
...
| `FL` | `#9c815b` warm sand | `#e48baa` | File / document / report |
...
Data-type chip colors are a **separate semantic axis** from the per-node color override (§4). Chip colors describe *payload format*; node color describes *concern type*. A node can have both an `out: TB` mustard chip and a muted pink border simultaneously.
...
nodes:
  ...
  - { lane: "SVY", step: 3,  title: "HQ Review",         sub: "Submissions → Approved",
      tool: "Survey Sol. HQ",              chips: {in: "DB", out: "DB"},  color: "#c15b80" }   # muted pink · governance
...
- `assets/example-process-extended.html` — exercises §4 color override: Build app in slate-blue (data quality), Train enumerators in muted pink (governance), Publish results in olive-green (data products). Focal accent on Pilot test step + node unchanged.
