...
`C_light` = the same hex lightened ~15 % for dark-mode contrast (e.g., `#7a8c47` → `#9aac67`, `#c15b80` → `#d97a78`).
...
- `#c15b80` muted pink — security / governance / pain-point that isn't focal
...
- **Using `kind: focal` to flag every painful thing.** Focal exists for ≤ 2 narrative pain-points; for "this is bad but not headline-bad", use `color: "#c15b80"` muted pink instead.
