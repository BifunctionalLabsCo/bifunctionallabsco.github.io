...
- Circle fill: very low-opacity tint — `rgba(3,43,34,0.04)` for ink set, `rgba(0,99,77,0.05)` for muted. Tints compound naturally in overlap regions.
...
- **Set labels** placed outside the circle, NEVER crossing the stroke. Montserrat 12–14px 600 for the set name, optional system mono 9px sublabel.
- **Intersection labels** placed inside the overlap region, Montserrat 12px 600, centered. For small overlaps, use a leader line to a label in clear space.
- **accent accent** on the ONE focal intersection — the "sweet spot". Either accent label stroke OR clipPath-bounded accent fill tint (`rgba(255,79,129,0.10)`).
