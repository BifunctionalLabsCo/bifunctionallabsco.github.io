...
A **layout variant** of the standard quadrant — same house skin (warm paper, dot pattern, Fraunces title, Montserrat mono eyebrows, accent focal rule). The grammar shifts: axes hold a **range** rather than a measurement; cells hold **named scenarios** rather than positioned items.
...
- **Axis lines**: `ink` (`#032b22`), `stroke-width: 1.2`, `marker-start` + `marker-end` both pointing outward.
- **Focal quadrant tint**: `rgba(255,79,129,0.04)` full rect behind the focal cell.
...
- **Cell title**: Montserrat sans, 16px, weight 600, `ink`.
- **Cell description**: Montserrat sans, 11px, `muted`, 1–3 lines, left-aligned inside the cell.
...
- Sans-serif H1 — keep Fraunces for the page title. The title/diagram contrast is the house signature.
