...
chevrons:                       # ordered left → right; reserved names auto-promote to vertical
  ...
  - { name: "Security",             vertical: true, color: "#c15b80" }   # tinted to match the Identity bar below
...
components:                     # inside the cluster, plus bars and cross-cutting rows
  ...
  - { name: "Identity",   chevron: "Security",       kind: cross-cutting, icon: keycloak,   subtitle: "Keycloak · LDAP · OIDC",     color: "#c15b80" }
...
Fills alternate `#032b22` / `#10231d` (light mode) or `#10231d` / `#1a2f28` (dark mode). Labels: paper-colored mono `font-size=7`, `letter-spacing=0.14em`, `text-anchor=middle`, centered at `chevron_cx, 21`.

**Color override** (per chevron, both horizontal and vertical): a chevron may declare an optional `color: "#hex"` that replaces the alternation fill for that one chevron. Use it to flag a phase that pairs with a custom-colored component (e.g., `Security` chevron in red when the Identity bar uses `color: "#c15b80"`). Rules:
...
Stroke: `rgba(3,43,34,0.20)`, `stroke-width=0.8`, `stroke-dasharray=6,3`, `rx=6`. Zone fill: `rgba(3,43,34,0.02)`.
...
Stroke: `rgba(3,43,34,0.18)`, `stroke-width=1.2`, `rx=8`. Fill: `rgba(3,43,34,0.02)`. K8s icon + label at `(cluster_x + 16, 352)` (icon) and `(cluster_x + 40, 362)` (text).
...
Stroke: `rgba(3,43,34,0.20)`, `stroke-width=0.8`, `rx=6`. Fill: `rgba(3,43,34,0.05)`. Icon at `(16, cross_y(k) + 10)`, name centered at `(effective_w / 2, cross_y(k) + 22)`, subtitle at `(effective_w / 2, cross_y(k) + 34)`.
...
Stroke: `rgba(3,43,34,0.18)`, `stroke-width=0.8`, `rx=4`. Fill: `rgba(3,43,34,0.05)`. Tool icon at the far right (`bar_x + bar_w - 50, 58`); name centered at `(bar_x + bar_w/2, 71)`; subtitle at `(bar_x + bar_w/2, 84)`.
...
**Focal node:** `fill="rgba(255,79,129,0.08)"`, `stroke="#ff4f81"`, `stroke-width=1.2`. Title text in accent color. All other nodes: white fill, `stroke=rgba(3,43,34,0.25)`, `stroke-width=1`.
...
Fills alternate `#032b22` / `#10231d` (same palette as horizontals). Labels: paper-colored mono `font-size=7`, `letter-spacing=0.14em`, **rotated −90°**, anchored at `(strip_x + strip_w/2, (y0+y1)/2)`.
...
|---|---|---|---|---|---|
| `primary` | `#ff4f81` | 1.2 | — | `arrow-accent` | Every edge whose endpoint is the `focal` node. |
| `secondary` | `#00634d` | 1.0 | — | `arrow` | Default for source→component and component→component when neither endpoint is focal. |
| `trigger` | `#00634d` | 1.0 | `4,3` | `arrow-sm` | Every edge originating from a `kind: bar` component. |
| `query` | `rgba(3,43,34,0.30)` | 1.0 | `4,3` | `arrow` | Read-back edges (e.g., focal ↔ Trino). |

**Defs block** (required, exactly these four markers):

```svg
<defs>
  <marker id="arrow"        markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#00634d"/></marker>
  <marker id="arrow-accent" markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="#ff4f81"/></marker>
  <marker id="arrow-sm"     markerWidth="6" markerHeight="5" refX="5" refY="2.5" orient="auto"><polygon points="0 0, 6 2.5, 0 5" fill="#00634d"/></marker>
  <marker id="arrow-dim"    markerWidth="8" markerHeight="6" refX="7" refY="3"   orient="auto"><polygon points="0 0, 8 3, 0 6" fill="rgba(3,43,34,0.45)"/></marker>
</defs>
```
...
- `#c15b80` (muted pink) — Security / Identity (Keycloak, Vault)
...
| Token | Light | Dark |
|---|---|---|
| Page paper | `#efede7` | `#10231d` |
| Ink | `#032b22` | `#efede7` |
| Muted text | `#00634d` | `rgba(239,237,231,0.65)` |
| Chevron dark fill | `#032b22` | `#10231d` |
| Chevron light fill | `#10231d` | `#1a2f28` |
| Chevron label | `#efede7` | `#efede7` (unchanged) |
| Dashed border | `rgba(3,43,34,0.20)` | `rgba(239,237,231,0.22)` |
| Cluster border | `rgba(3,43,34,0.18)` | `rgba(239,237,231,0.18)` |
| Node fill | white | `rgba(239,237,231,0.06)` |
| Node stroke | `rgba(3,43,34,0.25)` | `rgba(239,237,231,0.20)` |
| Focal fill | `rgba(255,79,129,0.08)` | `rgba(255,131,167,0.12)` |
| Focal stroke | `#ff4f81` | `#ff83a7` |
| Accent connector | `#ff4f81` | `#ff83a7` |
| Dot pattern | `rgba(3,43,34,0.10)` | `rgba(239,237,231,0.10)` |
