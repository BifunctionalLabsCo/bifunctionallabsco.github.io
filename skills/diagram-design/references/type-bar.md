...
- **Y-axis gridlines:** 4–6 horizontal lines at regular intervals. Stroke `rgba(3,43,34,0.08)` (very faint), 0.8px. X-axis baseline at `rgba(3,43,34,0.25)`, 1px.
- **Y-axis labels:** right-aligned system mono 8px muted, at x=72 (8px left of the plot area).
- **X-axis labels:** centered below each bar, Montserrat sans 11px 600 for category names.
...
<rect x="X" y="Y" width="W" height="H" fill="#efede7"/>
<!-- Bar body -->
<rect x="X" y="Y" width="W" height="H" fill="rgba(0,99,77,0.15)" stroke="#00634d" stroke-width="1"/>
<!-- Value label above bar -->
<text x="X+W/2" y="Y-8" fill="#00634d" font-size="8" font-family="'system mono', monospace" text-anchor="middle">VALUE</text>
```

Focal bar: replace fill with `rgba(255,79,129,0.12)`, stroke with `#ff4f81`, label fill with `#ff4f81`.
